...
mp_per_cell_sqrts=("0.5" "1" "2" "3")
grid_densities=("64")
JULIA_THREADS=("48" "64")
TACC_RUNTIME=("16:00:00")

grid_r_min_active=("1.3" "1.4")
grid_r_max_active=("2.5" "2.3")
...
function send_batch() {
    ...
    local batch_name="batch_ex1_p${mp_per_cell_sqrt}_h${grid_density}.sh"
    ...
    echo "#SBATCH -J 1_${output_name}" >> $batch_name
    echo "#SBATCH -o 1_${output_name}.o%j" >> $batch_name
    echo "#SBATCH -e 1_${output_name}.e%j" >> $batch_name
    ...
    echo "" >> $batch_name
    echo "LD_LIBRARY_PATH=\"\" julia -t $threads -- analysis_new/catenoid/Main.jl \
--grid_num_cells $grid_num_cells $grid_num_cells $grid_num_cells \
--output_name $output_name/ \
--dest $dest \
--mesh_file $mesh_file \
--grid_r_min_active $min_active \
--grid_r_max_active $max_active" >> $batch_name
    echo "" >> $batch_name
...
do
    ...
    grid_min_active=${grid_r_min_active[i]}
    grid_max_active=${grid_r_max_active[i]}
    ...
    do
        ...
        send_batch $grid_density $mp_per_cell_sqrt $threads $runtime $grid_min_active $grid_max_active
        sleep 1
        ((sim_count+=1))
    done
done
