...
"""
function initialize2DGmshBulkAndSurface!(file_name::String)::Tuple{Vector{MaterialPoint}, Vector{SurfacePoint}, VTKLookup, VTKLookup}
    gmsh.initialize()
    gmsh.open(file_name)
...
function parseBulk!(coords::AbstractArray{Vec3{Float64}}, tag2idx::Dict{UInt64, Int64})::Tuple{Vector{MaterialPoint}, VTKLookup}
    ...
    quad_type::Int64 = gmsh.model.mesh.getElementType("Quadrangle", 1)
    bulk_entities::Vector{Int64} = gmsh.model.getEntitiesForPhysicalGroup(2, 1)
    ...
    for entity in bulk_entities
        element_node_tags::Vector{Int64}, _, _ = gmsh.model.mesh.getNodesByElementType(quad_type, entity)
        append!(bulk_node_tags, element_node_tags)

        n_quads::Int64 = length(element_node_tags) ÷ 4
        sizehint!(material_domain, length(material_domain) + n_quads)
    ...
    @threads for material_point::MaterialPoint in material_domain
        vertices::Vector{Vec2{Float64}} = Vector{Vec2{Float64}}(undef, 4)
        for (vertex_idx, vtk_tag) in enumerate(material_point.node_connectivity_vtk)
            vtk_vertex::Vec3{Float64} = coords[tag2idx[vtk_tag]]
            vertices[vertex_idx] = Vec2{Float64}(vtk_vertex[1], vtk_vertex[2])
        end
        material_point.vertices = vertices
        material_point.volume = material_point.volume_init = getQ4Area(vertices)
    end
...
function parseSurface!(coords::AbstractArray{Vec3{Float64}}, tag2idx::Dict{UInt64, Int64})::Tuple{Vector{SurfacePoint}, VTKLookup}
    line_type::Int64 = gmsh.model.mesh.getElementType("Line", 1)
    surface_entities::Vector{Int64} = gmsh.model.getEntitiesForPhysicalGroup(1, 2)
    ...
    for entity in surface_entities
        element_node_tags::Vector{Int64}, _, _ = gmsh.model.mesh.getNodesByElementType(line_type, entity)
        append!(surface_node_tags, element_node_tags)

        n_lines::Int64 = length(element_node_tags) ÷ 2
        sizehint!(surface_domain, length(surface_domain) + n_lines)

        for i in 1:2:length(element_node_tags)
            tag2, tag1 = element_node_tags[i:i+1] # ensures outward normal

            surface_point::SurfacePoint = SurfacePoint()
            surface_point.node_connectivity_vtk = Vec2{Int64}(tag1, tag2)
    ...
    @threads for surface_point::SurfacePoint in surface_domain
        vertices::Vector{Vec2{Float64}} = Vector{Vec2{Float64}}(undef, 2)
        for (vertex_idx, vtk_tag) in enumerate(surface_point.node_connectivity_vtk)
            vtk_vertex::Vec3{Float64} = coords[tag2idx[vtk_tag]]
            vertices[vertex_idx] = Vec2{Float64}(vtk_vertex[1], vtk_vertex[2])
        end
        surface_point.vertices = vertices
        surface_point.area_init, surface_point.normal_init = getL2AreaAndNormal(surface_point.vertices)
...
function initialize2DGmshRigidBody!(file_name::String)::Tuple{Vector{RigidPoint}, VTKLookup}
    gmsh.initialize()
    gmsh.open(file_name)
    ...
    rigid_domain::Vector{RigidPoint}, rigid_lookup::VTKLookup = parseRigidBulk!(coords, tag2idx)
...
function parseRigidBulk!(coords::AbstractArray{Vec3{Float64}}, tag2idx::Dict{UInt64, Int64})::Tuple{Vector{RigidPoint}, VTKLookup}
    ...
    for entity in bulk_entities
        ...
        n_quads::Int64 = length(element_node_tags) ÷ 4
        sizehint!(rigid_domain, length(rigid_domain) + n_quads)
...
struct BulkVTKData
    zero_cell::Array{Float64, 1} # used for filling in blank field values in the VTK files
...
function initializeVTKBulkData(material_domain::Vector{MaterialPoint}, destination_folder::String)::Tuple{BulkVTKData, WriteVTK.CollectionFile, Vector{MeshCell}}
    ...
    bulk_data = BulkVTKData(
        ...
        Array{Float64}(undef, numMP),
        Array{Float64}(undef, numMP),
        Array{Float64}(undef, numMP, 2),
        Array{Float64}(undef, numMP, 2, 2),
        Array{Float64}(undef, numMP, 2, 2)
    )
    pvd_bulk = paraview_collection(joinpath(destination_folder, "bulk"))
    vtk_bulk_cells = [MeshCell(VTKCellTypes.VTK_QUAD, material_domain[mp_index].node_connectivity_vtk) for mp_index = 1:numMP]
...
function initializeVTKSurfaceData(surface_domain::Vector{SurfacePoint}, destination_folder::String)::Tuple{SurfaceVTKData, WriteVTK.CollectionFile, Vector{MeshCell}}
    ...
    surface_data = SurfaceVTKData(
        ...
        Array{Float64}(undef, numSP),
        Array{Float64}(undef, numSP, 2)
    )
    pvd_surf = paraview_collection(joinpath(destination_folder, "surf"))
    vtk_surf_cells = [MeshCell(VTKCellTypes.VTK_LINE, surface_domain[sp_index].node_connectivity_vtk) for sp_index = 1:numSP]
...
function initializeVTKRigidData(rigid_domain::Vector{RigidPoint}, destination_folder::String)::Tuple{WriteVTK.CollectionFile, Vector{MeshCell}}
    ...
    vtk_rigid_cells = [MeshCell(VTKCellTypes.VTK_QUAD, rigid_domain[rp_index].node_connectivity_vtk) for rp_index = 1:numRP]
