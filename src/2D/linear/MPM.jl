...
function materialToGrid!(grid::Grid, material_domain::Vector{MaterialPoint}, t::Float64, t_load::Float64)::Nothing
    ...
    @threads for material_point::MaterialPoint in material_domain
        for connected_grid_index::Int8 in 1:4
            ...
            shape_value::Float64, shape_gradi::Vec2{Float64} = linearFunctionAndGradient(material_point.position,
...
function updateGrid!(grid::Grid, dt::Float64)::Nothing
    @threads for grid_point::GridPoint in grid.points
        ...
        if (grid_point.is_fixed[2] == true)
            ...
            grid_point.force = Vec2{Float64}(grid_point.force[1], 0.0) # kind of unnecessary to zero-out the force...?
        end
...
function gridToMaterial!(material_domain::Vector{MaterialPoint}, grid::Grid, dt::Float64, alpha::Float64)::Nothing
    @threads for material_point::MaterialPoint in material_domain
        ...
        for connected_grid_index::UInt8 in 1:4
            ...
            grid_velocity_sum += shape_value .* adj_grid_point.velocity
...
function gridToMaterialNoFBar!(material_domain::Vector{MaterialPoint}, grid::Grid, dt::Float64, alpha::Float64, applyConstitutiveEquation!::Function)::Nothing
    @threads for material_point::MaterialPoint in material_domain
        grid_velocity_sum::Vec2{Float64} = ZERO_VEC2
        grid_velocity_increment_sum::Vec2{Float64} = ZERO_VEC2
        velocity_gradient::Mat22{Float64} = ZERO_MAT22
...
#         material_point.connected_grid_indices = findConnectedGrid(material_point.position,
...
#             shape_value::Float64 = linearFunction(material_point.position, 
#                                                     adj_grid_point.position, projection_grid.cell_length)
...
function updateMaterialPositions!(material_domain::Vector{MaterialPoint}, grid::Grid, dt::Float64, update_connected_grid::Bool)::Nothing
    @threads for material_point::MaterialPoint in material_domain
        ...
        for grid_index::UInt32 in material_point.connected_grid_indices
            ...
            shape_value::Float64 = linearFunction(material_point.position, grid_point.position, grid.cell_length)
