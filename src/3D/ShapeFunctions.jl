...
function bulkCPDIShapeFunctionAndGradient(material_vertices::Vector{Vec3{Float64}}, nodal_pos::Vec3{Float64}, nodal_width::Vec3{Float64})::Tuple{Float64, Vec3{Float64}}
    # points
    x1::Float64 = material_vertices[1][1]; y1::Float64 = material_vertices[1][2]; z1::Float64 = material_vertices[1][3]
    x2::Float64 = material_vertices[2][1]; y2::Float64 = material_vertices[2][2]; z2::Float64 = material_vertices[2][3]
    x3::Float64 = material_vertices[3][1]; y3::Float64 = material_vertices[3][2]; z3::Float64 = material_vertices[3][3]
    x4::Float64 = material_vertices[4][1]; y4::Float64 = material_vertices[4][2]; z4::Float64 = material_vertices[4][3]

    # N_I
    linear1::Float64 = linearFunction(material_vertices[1], nodal_pos, nodal_width)
    linear2::Float64 = linearFunction(material_vertices[2], nodal_pos, nodal_width)
    linear3::Float64 = linearFunction(material_vertices[3], nodal_pos, nodal_width)
    linear4::Float64 = linearFunction(material_vertices[4], nodal_pos, nodal_width)

    # geometric volume
    J::Float64 = (x2-x1)*((y2-y3)*(z3-z4) - (y3-y4)*(z2-z3)) + 
                 (x3-x2)*((y3-y4)*(z1-z2) - (y1-y2)*(z3-z4)) + 
                 (x4-x3)*((y1-y2)*(z2-z3) - (y2-y3)*(z1-z2))

    # gradients
    grad_1::Vec3{Float64} = Vec3{Float64}((y4-y2)*(z3-z2) - (y3-y2)*(z4-z2),
                                          (x3-x2)*(z4-z2) - (x4-x2)*(z3-z2),
                                          (x4-x2)*(y3-y2) - (x3-x2)*(y4-y2))
    grad_2::Vec3{Float64} = Vec3{Float64}((y3-y1)*(z4-z3) - (y3-y4)*(z1-z3),
                                          (x4-x3)*(z3-z1) - (x3-x1)*(z4-z3),
                                          (x3-x1)*(y4-y3) - (x3-x4)*(y1-y3))
    grad_3::Vec3{Float64} = Vec3{Float64}((y2-y4)*(z1-z4) - (y1-y4)*(z2-z4),
                                          (x1-x4)*(z2-z4) - (x2-x4)*(z1-z4),
                                          (x2-x4)*(y1-y4) - (x1-x4)*(y2-y4))
    grad_4::Vec3{Float64} = Vec3{Float64}((y1-y3)*(z2-z1) - (y2-y1)*(z1-z3),
                                          (x2-x1)*(z1-z3) - (x1-x3)*(z2-z1),
                                          (x1-x3)*(y2-y1) - (x2-x1)*(y1-y3))

    return (0.25 * (linear1 + linear2 + linear3 + linear4)), 
            ((1 / J) * (linear1 * grad_1 + linear2 * grad_2 + linear3 * grad_3 + linear4 * grad_4))
end

function getTet4Volume(material_vertices::Vector{Vec3{Float64}})::Float64
    # points
    x1::Float64 = material_vertices[1][1]; y1::Float64 = material_vertices[1][2]; z1::Float64 = material_vertices[1][3]
    x2::Float64 = material_vertices[2][1]; y2::Float64 = material_vertices[2][2]; z2::Float64 = material_vertices[2][3]
    x3::Float64 = material_vertices[3][1]; y3::Float64 = material_vertices[3][2]; z3::Float64 = material_vertices[3][3]
...
function surfCPDITri3ShapeFunctionAndGradient(surface_vertices::Vector{Vec3{Float64}}, nodal_pos::Vec3{Float64}, nodal_width::Vec3{Float64})::Tuple{Float64, Vec3{Float64}}
    ...
    pg::Vec3{Float64} = (p1 + p2 + p3) / 3
    S_1::Float64 = dot(n, cross(p2, p3) + cross(pg, p2-p3))
    S_2::Float64 = dot(n, cross(p3, p1) + cross(pg, p3-p1))
    S_3::Float64 = dot(n, cross(p1, p2) + cross(pg, p1-p2))
...
function surfCPDITri3ShapeGradient(surface_vertices::Vector{Vec3{Float64}}, nodal_pos::Vec3{Float64}, nodal_width::Vec3{Float64})::Vec3{Float64}
    # points
    p1::Vec3{Float64} = surface_vertices[1]; p2::Vec3{Float64} = surface_vertices[2]; p3::Vec3{Float64} = surface_vertices[3]
