...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Creates a 2D mesh of a square.")
...
def create_mesh(args):
    ...
    gmsh.initialize()
    gmsh.model.add("SquareDroplet")

    mesh_density = args.mesh_density
    file_name = f"{args.output}square_msh{mesh_density}.msh"
    
    print("MSHPY | Creating the square geometry")
    gmsh.model.occ.addRectangle(0.0, 0.0, 0.0, 1.0, 1.0, 1)
    gmsh.model.occ.synchronize()

    gmsh.model.mesh.setTransfiniteCurve(1, mesh_density + 1)
    gmsh.model.mesh.setTransfiniteCurve(2, mesh_density + 1)
    gmsh.model.mesh.setTransfiniteCurve(3, mesh_density + 1)
    gmsh.model.mesh.setTransfiniteCurve(4, mesh_density + 1)
