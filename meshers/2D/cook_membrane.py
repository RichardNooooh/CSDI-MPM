...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Creates a 2D mesh of Cook's Membrane.")
    parser.add_argument('-m', '--mesh_density', type=float, default=0.5, help='the \"density\" of the mesh - defines the number of elements on each side of the square') 
    ...
    parser.add_argument('--subdiv_algorithm', type=int, default=1, help='the GMSH subdivision algorithm number (see gmsh.info for details)')   
...
def create_mesh(args):
    ...
    gmsh.initialize()
    gmsh.model.add("CookMem")

    file_name = args.output + "cook" +"_msh" + str(args.mesh_density) + ".msh"

    print("MSHPY | Creating the cook membrane geometry")
    gmsh.model.occ.addPoint(0.0, 0.0, 0.0, tag=1)
    gmsh.model.occ.addPoint(48.0, 44.0, 0.0, tag=2)
    gmsh.model.occ.addPoint(48.0, 60.0, 0.0, tag=3)
    gmsh.model.occ.addPoint(0.0, 44.0, 0.0, tag=4)

    gmsh.model.occ.addLine(1, 2, tag=1)
    gmsh.model.occ.addLine(2, 3, tag=2)
    gmsh.model.occ.addLine(3, 4, tag=3)
    gmsh.model.occ.addLine(4, 1, tag=4)

    gmsh.model.occ.addCurveLoop([1, 2, 3, 4], tag=1)
    gmsh.model.occ.addPlaneSurface([1], tag=1)

    gmsh.model.occ.synchronize()
    gmsh.model.mesh.setSize(gmsh.model.getEntities(0), 1 / args.mesh_density)

    gmsh.model.mesh.setTransfiniteCurve(1, int(math.floor(60 * args.mesh_density)))
    gmsh.model.mesh.setTransfiniteCurve(3, int(math.floor(60 * args.mesh_density)))
