...
_arguments = Dict{String,Any}([
    ...
    ("constitutive", "fluid"),
    ("mesh_file", "meshes/3D/cube_msh16.msh"),
    ("grid_dims", [1.0, 1.0, 1.0]), # mm x mm
    ("grid_num_cells", [16, 16, 16]),
    ("dest", "output/review/"),
    ("output_name", "cube_ppc1_h16/"),
    ("t_init", 0.0), # ms
    ("t_delta", 1e-3), # ms
    ("t_final", 1000.0), # ms
    ("t_load", 200),
...
function run(arguments::Dict{String,Any})::Nothing
    ...
    lookup_tables::Tuple{VTKLookup,VTKLookup} = initializeBodyMeshes(arguments, grid, setBulkParameters!, setSurfaceParameters!; get_active_subset=true)
...
function setSurfaceParameters!(surface_domain::Vector{SurfacePoint}, grid::Grid, arguments::Dict{String,Any})::Nothing
    ...
    @threads for surface_point::SurfacePoint in surface_domain
        # Symmetry Planes x = 0, y = 0 and z = 0
        if getSumOfVertexInDimension(surface_point, 1) < EPSILON || getSumOfVertexInDimension(surface_point, 2) < EPSILON || getSumOfVertexInDimension(surface_point, 3) < EPSILON
